...
class AdminService:
    ...
    def block_user(self, user_id: int) -> BlockResponseDTO:
        ...
        logger.warning({"event": "user_blocked_antifraud", "user_id": user_id, "timestamp": datetime.now(timezone.utc).isoformat()})
    ...
    def get_compliance_report(self) -> List[Dict[str, Any]]:
        logger.info({"event": "compliance_report_requested", "timestamp": datetime.now(timezone.utc).isoformat()})
