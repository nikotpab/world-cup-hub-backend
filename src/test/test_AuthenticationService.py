...
class TestAuthenticationService:
    ...
    def test_register_success(self, mock_get_db):
        ...
        success, message = AuthenticationService.register(
            "nuevo_pro", "password123", "test@mundial.com"
        )
    ...
    def test_register_invalid_email(self, mock_get_db):
        """Prueba registro con formato de email incorrecto"""
        success, message = AuthenticationService.register(
            "user", "pass", "esto-no-es-un-email"
        )
    ...
    def test_register_user_exists(self, mock_get_db):
        """Prueba error cuando el nombre de usuario ya está tomado"""
        mock_conn = MagicMock()
        mock_cur = mock_conn.cursor.return_value
        mock_get_db.return_value = mock_conn
    ...
    def test_login_wrong_password(self, mock_get_db):
        """Prueba login fallido por contraseña errónea"""
        mock_conn = MagicMock()
        mock_cur = mock_conn.cursor.return_value
        mock_get_db.return_value = mock_conn
    ...
    def test_login_user_not_found(self, mock_get_db):
        """Prueba login con un usuario que no existe en la DB"""
        mock_conn = MagicMock()
        mock_cur = mock_conn.cursor.return_value
        mock_get_db.return_value = mock_conn
